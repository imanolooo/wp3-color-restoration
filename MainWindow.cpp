...
void MainWindow::loadPickedColors() {
    ...
    _finalColors.emplace("dark grey", PickedColor("dark grey", "carlos", -1, -1, 0, 44, 43, 56));
    ...
    _finalColors.emplace("medium grey", PickedColor("medium grey", "carlos", -1, -1, 0, 33, 32, 32));
...
void MainWindow::updateFinalColorsGUI() {
    ...
    for(const auto &pickedColor : _pickedColors[colorName.toStdString()]) {
        ro += pickedColor.red();
        go += pickedColor.green();
        bo += pickedColor.blue();
    }
    ro /= _pickedColors[colorName.toStdString()].size();
    go /= _pickedColors[colorName.toStdString()].size();
    bo /= _pickedColors[colorName.toStdString()].size();

    color::rgb<float> rgb({r, g, b});
    color::lab<float> lab;
    lab = rgb;

    color::rgb<float> rgbo({ro, go, bo});
    ...
    r_str = QString::number(rgb[0]);
    g_str = QString::number(rgb[1]);
    b_str = QString::number(rgb[2]);
